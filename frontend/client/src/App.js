...
export default function CyberThreatDetector() {
  ...
  const validateInput = (value) => urlRegex.test(value) || hashRegex.test(value) || ipRegex.test(value);
  ...
  const localThreatCheck = (value) => {
    ...
    if (unsafeList.includes(val)) {
      return { status: "⚠️ Threat detected!", stats: { malicious: 5 }, total_engines: 10 };
    } else if (suspiciousList.includes(val)) {
      return { status: "⚠️ Suspicious!", stats: { malicious: 1 }, total_engines: 10 };
    } else {
      return { status: "✅ No threat found", stats: { malicious: 0 }, total_engines: 10 };
    }
  };
  ...
  const fetchHistory = async () => {
    try {
      ...
      const mapped = res.data.map((scan) => ({
        input: scan.input,
        status: scan.status,
        result: scan,
      }));
  ...
  const getStatusStyle = (status) => {
    ...
    if (status?.includes("Not found")) return "bg-gray-500/20 text-gray-300 border border-gray-500/30";
  ...
  const getStatusIcon = (status) => {
    ...
    if (status?.includes("Not found")) return <Shield className="text-gray-400 w-8 h-8" />;
      ...
      <div className="w-full max-w-md bg-gray-900/80 shadow-2xl p-6 rounded-3xl flex flex-col gap-4">
        ...
        <button
          ...
          className={`w-full py-3 rounded-xl font-semibold transition flex justify-center items-center gap-2 
            ${loading || !validateInput(input.trim()) ? "bg-gray-600 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-500"}`}
        >
        ...
        <div className="w-full max-w-md bg-gray-900/80 shadow-2xl p-6 rounded-3xl mt-6 text-center">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="mt-1 text-gray-400 text-sm">
              Engines: {result.total_engines ?? 0} | Malicious: {result.stats?.malicious ?? result.stats?.abuseConfidenceScore ?? 0}
            </p>
          ...
          <ul className="flex flex-col gap-2">
            {history.map((h, i) => (
              <li
                ...
                className="flex items-center justify-between p-3 bg-gray-800/60 rounded-xl hover:bg-gray-700/70 transition cursor-pointer"
              >
